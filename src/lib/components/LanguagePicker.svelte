...
>
	<div
		class="absolute top-1 left-0 h-[calc(100%-0.5rem)] rounded-full bg-zinc-100 shadow transition-[transform,width] duration-300 ease-out"
	...
	{#each supportedLocales as lang, i (lang)}
		<button
			...
			class={`relative z-10 cursor-pointer rounded-full px-3 py-1.5 text-xs font-semibold tracking-wider uppercase transition-colors duration-300 ${
				i18n.locale === lang
					? 'text-zinc-900'

<script lang="ts">
	...
	const skills = [
		{
			name: m.skill_web_mobile(),
			items: ['Angular', 'Vue', 'SvelteKit', 'TypeScript', 'Tailwind CSS', 'Flutter']
		},
		{
			name: m.skill_backend_db(),
			items: ['.NET C#', 'Python', 'Java', 'Node.js', 'PostgreSQL', 'MSSQL', 'MongoDB']
		},
		{
			name: m.skill_tools_platforms(),
			items: ['AWS', 'Azure', 'Cloudflare', 'Docker', 'Linux', 'Supabase', 'Firebase']
		}
	];
			...
			<div>
				...
				<div class="space-y-5 text-base leading-8 text-zinc-300 sm:text-lg">
					<p>
						...
						>{m.about_para1_sep2()}<span use:shine class="word-shine">{m.about_para1_word3()}</span
						>{m.about_para1_suffix()}
					</p>
					<p>
							...
							<div class="relative flex gap-4 pb-10 last:pb-0">
								...
								<div class="flex-1">
									<div class="mb-2 flex flex-wrap items-baseline justify-between gap-2">
										<h4
											use:shine={{ hitTest: true }}
											class="title-shimmer text-xl font-semibold text-white"
										>
											{exp.role}
										</h4>
									...
									<p class="mt-2 text-sm font-medium text-zinc-500">
										{exp.tech_stack} C#, ASP.NET, Angular, TypeScript, Python, PostgreSQL, MSSQL, AWS,
										Azure, Kubernetes
									</p>
								</div>
							</div>
			...
			<div class="lg:sticky lg:top-8">
				...
				<div class="space-y-3 sm:space-y-6">
					{#each skills as skillGroup (skillGroup.name)}
						<div
							use:shine
							class="box-shine rounded-2xl border border-zinc-800 bg-zinc-900/70 p-5 shadow-lg shadow-black/20 transition hover:border-zinc-600 sm:p-6"
						>
							<h4
								use:shine={{ within: '.box-shine' }}
								class="link-shine shine-inherit mb-3 text-base font-semibold text-zinc-200 sm:mb-4 sm:text-xl"
							>
								{skillGroup.name}
							</h4>
							<div class="flex flex-wrap gap-2">
								{#each skillGroup.items as skill (skill)}
									<span
										use:shine
										class="btn-shine pill-shine cursor-default rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-1.5 text-[13px] font-medium text-zinc-300 transition hover:border-zinc-600 hover:text-white sm:px-3 sm:py-2 sm:text-sm"
									>

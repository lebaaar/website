<script lang="ts">
	...
	let {
		...
		iconBg?: string;
		iconShadow?: string;
		media?: (string | { src: string; poster?: string; type?: 'image' | 'video'; label?: string })[];
			...
			<div class="mb-6 flex items-center gap-5">
				...
				<div>
					...
					<p class="mt-2 flex flex-wrap items-center gap-2 text-sm text-zinc-400">
						...
						{#if badge}
							<span
								class="rounded-full border border-zinc-600/50 bg-zinc-700/25 px-2 py-0.5 text-xs text-zinc-300"
							>
								{badge}
							</span>
						{/if}
		...
		{#snippet featuresBlock()}
			...
			<ul class="space-y-6">
				{#each features as feature (feature.title)}
					<li class="flex gap-4">
						<svg
							class="mt-1 h-5 w-5 shrink-0 text-zinc-500"
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 24 24"
							fill="none"
							stroke="currentColor"
						...
						<div>
							<h3
								use:shine={{ hitTest: true }}
								class="title-shimmer text-lg font-semibold text-white"
							>
								{feature.title}
							</h3>

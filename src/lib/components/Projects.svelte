...
>
	...
	<div class="mb-10 flex justify-center sm:mb-14" role="tablist" aria-label={m.projects_title()}>
		...
		>
			<div
				class="absolute top-0 left-0 rounded-xl bg-zinc-100 shadow transition-[transform,width,height] duration-300 ease-out"
			...
			{#each filters as filter, i (filter.value)}
				<button
					...
					bind:this={tabEls[i]}
					class={`relative z-10 cursor-pointer rounded-xl px-4 py-2 text-sm font-medium transition-colors duration-300

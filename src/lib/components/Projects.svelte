			...
			{#each filters as filter, i (filter.value)}
				<button
					...
					${activeFilter === filter.value
						? 'text-zinc-900'
						: 'btn-shine text-zinc-400 hover:text-white'}`}
		...
		{#each filteredProjects as project, i (project.title)}
			...
			>
				...
				<div class="mb-6 flex gap-3 items-center">
					...
					<div class="flex-1">
						<h3 use:shine={{ within: '.group', hitTest: true }} class="title-shimmer text-2xl font-bold tracking-tight text-white">{project.title}</h3>
					</div>
				</div>
				...
				<div class="relative z-10 mt-auto flex flex-wrap gap-2">
					{#if project.slug}
						<!-- eslint-disable-next-line svelte/no-navigation-without-resolve -->
						<a use:shine href={`/projects/${project.slug}`}
					...
					{#if project.github && !(project.slug && project.link)}
						<!-- eslint-disable-next-line svelte/no-navigation-without-resolve -->
						<a use:shine href={project.github} target="_blank" rel="noopener noreferrer" class={`${projectButtonClass} hover:bg-zinc-800`}>
